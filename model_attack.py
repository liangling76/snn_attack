...
def set_thresh_pernultimate(thresh_value):
    global thresh_pernultimate
    thresh_pernultimate = thresh_value
...
class ActFunThresh(torch.autograd.Function):

    @staticmethod
    def forward(ctx, input):
        global thresh_pernultimate
        ctx.save_for_backward(input)
        return input.gt(thresh_pernultimate).float()
...
act_fun_thresh = ActFunThresh.apply


def mem_update(conv, x, mem, spike, modithresh=False):
    mem = mem * decay * (1. - spike) + conv(x)
    if modithresh:
        spike = act_fun_thresh(mem)
    else:
        spike = act_fun(mem)
    return mem, spike
...
class MNIST(nn.Module):
    ...
    def forward(self, input, time_window=15):
        ...
        for step in range(time_window):
            ...
            h2_mem, h2_spike = mem_update(self.fc2, x,        h2_mem, h2_spike, modithresh=True)
